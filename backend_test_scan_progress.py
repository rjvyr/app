...
class ScanProgressTest(unittest.TestCase):
    ...
    def test_03_start_scan_and_check_progress(self):
        ...
        if progress_response.status_code == 200:
            ...
            # Check if progress data has expected fields
            ...
            progress_response2 = requests.get(f"{self.base_url}/api/scans/{scan_id}/progress", headers=headers)
            if progress_response2.status_code == 200:
                progress_data2 = progress_response2.json()
                print(f"Updated progress data: {json.dumps(progress_data2, indent=2)}")
                
                # Check if progress has increased
                if progress_data2["status"] != "completed":
                    self.assertGreater(progress_data2["progress"], initial_progress, 
                                      "Progress should increase over time")
            else:
                print(f"Second progress check failed with status code: {progress_response2.status_code}")
        else:
            print(f"Progress endpoint not found or not working. Status code: {progress_response.status_code}")
            if progress_response.status_code != 404:
                print(f"Response content: {progress_response.text}")
        
    def test_04_check_scan_progress_collection(self):
        """Check if scan_progress collection exists and is being updated"""
        if not hasattr(self.__class__, 'token') or not hasattr(self.__class__, 'brand_id'):
            self.skipTest("Previous tests failed, skipping this test")
        ...
        # Check if scan results contain progress information
        self.assertIn("scan_id", scan_result)
        ...
        # Check if the most recent scan has results
        latest_scan = scan_results_data["scans"][0]
        self.assertIn("results", latest_scan)
        self.assertTrue(len(latest_scan["results"]) > 0)
        
        print("✅ Scan execution and results verification successful")
