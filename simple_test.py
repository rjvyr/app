...
if not upraised_brand:
    ...
    upraised_brand_id = upraised_brand["_id"]

# Run a scan for the Upraised brand
print(f"Running scan for Upraised brand (ID: {upraised_brand_id})...")
scan_data = {
    "brand_id": upraised_brand_id,
    "scan_type": "quick"
}
...
domains_response = requests.get(f"{base_url}/api/source-domains?brand_id={upraised_brand_id}", headers=headers)
...
dashboard_response = requests.get(f"{base_url}/api/dashboard/real?brand_id={upraised_brand_id}", headers=headers)
